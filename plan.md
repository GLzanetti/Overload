...
│   │   ├── config/              # GatewayProperties (Map<String,String> de rotas por prefixo)
│   │   ├── router/              # GatewayController (catch-all "/**") e RouterService (monta/dispara chamada via RestClient)
│   │   └── security/            # RotasPublicasProperties, JwtService (valida token), JwtAuthFilter,
│   │                             #   RequestComUserId (HttpServletRequestWrapper) e JwtFilterConfig (registro via FilterRegistrationBean)
...
│   │   ├── model/ (entidades)
│   │   ├── dto/
│   │   ├── security/          # JWT (emissão), filtros de autenticação
...
1. **Roteamento**: um `Map<String, String>` relacionando prefixo de path → endereço interno do serviço (ex: `usuarios -> http://localhost:8081`, `auth -> http://localhost:8081`, uma entrada por prefixo real de endpoint, mesmo quando dois prefixos apontam para o mesmo serviço). Escolhido em vez de `if/else` porque é mais alinhado ao Open/Closed Principle — adicionar um serviço novo vira "adicionar uma entrada no Map", sem alterar a lógica de roteamento já existente. Configurado externamente via `application.yml` (`gateway.rotas`), lido por `GatewayProperties` (`config/`, `@ConfigurationProperties`). `GatewayController` (`router/`) é um catch-all `@RequestMapping("/**")` sem `method` fixo, que captura verbo, path, `Content-Type` e corpo (`byte[]`, tratado como conteúdo genérico — o Gateway nunca desserializa nem conhece os DTOs dos serviços internos) e repassa para `RouterService` (`router/`), que extrai o prefixo do path, resolve o endereço no `Map` e monta/dispara a chamada via `RestClient`, propagando `Content-Type` e desabilitando o lançamento de exception do `RestClient` em respostas de erro (`onStatus(status -> true, ...)`), para que o Gateway repasse status e corpo de erro do serviço interno (ex: `409` com `ErrorResponseDTO`) sem tratá-los como falha própria.
2. **Validação do JWT**: um `OncePerRequestFilter` (`JwtAuthFilter`, em `security/`) intercepta toda requisição, checa primeiro se o path está numa lista de rotas públicas (`RotasPublicasProperties`, `security/`, configurada via `gateway.rotas-publicas` no `application.yml` — hoje `/api/usuarios` e `/api/auth`); se estiver, deixa passar sem validar. Caso contrário, extrai o header `Authorization`, remove o prefixo `"Bearer "`, e delega a validação da assinatura/expiração e a extração do `sub` (ID do usuário) para `JwtService` (`security/`), que usa `jjwt` e devolve o ID diretamente (deixando `JwtException` subir e ser tratada pelo próprio filtro). Se o header estiver ausente/malformado ou o token for inválido, a requisição é cortada ali mesmo com 401, sem nunca chegar ao serviço de destino.
3. **Propagação da identidade**: o Gateway **não repassa o JWT original** para os serviços internos (eles não sabem lidar com token). Em vez disso, o filtro envolve a requisição original num `RequestComUserId` (`HttpServletRequestWrapper`, em `security/`) que sobrescreve `getHeader("X-User-Id")` para devolver o ID já extraído, e repassa essa versão decorada adiante na cadeia (`filterChain.doFilter`). O `GatewayController` lê esse header como leria qualquer outro e o `RouterService` o inclui explicitamente na chamada de saída via `RestClient`. Do lado do serviço de destino, isso é lido com um simples `@RequestHeader("X-User-Id")` no controller — nenhuma dependência de Spring Security ou JWT necessária ali. O registro do filtro na cadeia do Servlet é feito manualmente por `JwtFilterConfig` (`security/`, `@Configuration` com `FilterRegistrationBean<JwtAuthFilter>`), em vez de `@Component` no próprio filtro — escolha deliberada para manter o "encaixe" do filtro explícito e evitar registro duplicado.
...
1. **Setup inicial**: estrutura de pastas vazia, `docker-compose.yml` esqueleto, `.gitignore` (não esquecer de ignorar `target/`, `.env`, arquivos de IDE, e `entities.md` se optar por não publicar a modelagem detalhada) → primeiro commit.
...
- Modelagem detalhada de cada banco de dados — movida para um arquivo próprio, `entities.md` (fora do escopo deste plano de arquitetura; recomendado manter fora do repositório público — ver nota no próprio arquivo)
